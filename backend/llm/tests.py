...
class GenerateScriptViewTestCase(APITestCase):
    ...
    def test_successful_fallback_generation_flow(self):
        ...
        full_script = response.data.get("fullScript", "") or response.data.get("full_script", "")
        self.assertIn("[Customer Name]", full_script)
        self.assertIn("[Appointment Date]", full_script)
        
        response_handling = response.data.get("responseHandling", "") or response.data.get("response_handling", "")
        self.assertIn("10", response_handling.lower())
        
        closing = response.data.get("closing", "")
        self.assertTrue("opt-out" in closing.lower() or "opt out" in closing.lower() or "press 9" in closing.lower())
