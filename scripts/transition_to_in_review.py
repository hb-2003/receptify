...
# Subtasks of Week 1 that we want to move to 'In Review'
WEEK_1_SUBTASKS = [
    'KAN-7',   # DB migration
    ...
    'KAN-15',  # WEEK 1 BUFFER & TESTING GATE
]
...
def transition_issue(key, transition_id):
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            print(f"Successfully transitioned {key} to In Review (Transition ID: {transition_id})")
...
def main():
    print("Transitioning completed Receptify tickets to In Review status...")
    for key in WEEK_1_SUBTASKS:
        # Move back to In Review
        transition_issue(key, 31) # Transition ID 31 is 'In Review'
    
    print("\nStatus transition to In Review complete!")
